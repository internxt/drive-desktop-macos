...
/// creating them
func defaultWindows(authManager: AuthManager, usageManager: UsageManager, backupsService: BackupsService, updater: SPUUpdater, closeSendFeedbackWindow: @escaping () -> Void, closeFolderSelectorWindow: @escaping () -> Void, finishOrSkipOnboarding: @escaping () -> Void) -> [WindowConfig] {
    let windows = [
        WindowConfig(
        ...
        WindowConfig(
            view: AnyView(AppSettingsManagerView { FolderSelectorView(backupsService: backupsService, closeWindow: closeFolderSelectorWindow) }),
            title: nil,
            id: "folder-selector",
            width: 480,
            height: 380
